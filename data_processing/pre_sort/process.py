...
class PreProcessSess(object):
    ...
    def __init__(self, sess_prm_ffn, force_reprocess=False):
        ...
        self.parameters = params.get_params(sess_prm_ffn)
        self.prb = load_probe(os.path.join(self.path, self.parameters['prb_file']))
        write_probe(self.prb, os.path.join(self.path, self.parameters['prb_file']))
        self.rec_prm_fns = {}
    ...
    def generate_rec_prms(self):
        ...
        for rec, runs in self.parameters['raw_data_files'].iteritems():
            ...
            f.write(params.pydict_to_python(tmp_rec_prms))
            f.close()
        return
...
class PreProcessRec(object):
    ...
    def __init__(self, prm_ffn, **kwargs):
        ...
        self.parameters = params.get_params(filename=prm_ffn)
        self.prb = load_probe(os.path.join(self.path, self.parameters['prb_file']))
        write_probe(self.prb, os.path.join(self.path, self.parameters['prb_file']))
    ...
    def update_prm_file(self):
        ...
        prms_str = params.pydict_to_python(self.parameters)
...
def write_probe(prb, filename):
    ...
    prbstr = params.pydict_to_python(prbdict)
...
def main():
    ...
    prms = params.get_params(args.prm_filename)
    if type(prms['raw_data_files']) is dict and len(prms['raw_data_files']) > 1:
